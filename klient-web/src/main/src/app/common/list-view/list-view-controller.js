angular.module('nha.common.list-view', [
  ...
  .controller('ListCtrl', function HomeController($rootScope, $scope, $location, listService, httpService, errorService,
                                                  modalService, $filter, registerService, pagerService) {
    ...
    $scope.actionRemoveMedicalRecord = function (medicalRecord) {
      modalService.deleteModal($scope.text.pasientjournal, medicalRecord.navn + " (" + medicalRecord.fodselsnummer + ") ",
        function () {
    ...
    $scope.actionEditRecord = function (medicalRecord) {
      httpService.get(baseEndpointUrl + medicalRecord)
        .then(function(response) {
          registerService.setPasientjournalDTO(response.data);
